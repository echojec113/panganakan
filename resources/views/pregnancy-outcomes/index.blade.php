    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="border-b border-gray-100 px-6 py-4 bg-gray-50 flex flex-wrap items-center gap-2">
                <a href="{{ route('pregnancy-outcomes.index') }}" class="inline-flex items-center rounded-lg px-3 py-1.5 text-xs font-medium {{ $stateFilter === '' ? 'bg-[#55B85A] text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }}">All</a>
                @foreach(\App\Services\PregnancyOutcomeMonitoringService::STATE_FILTERS as $slug => $stateKey)
                    <a href="{{ route('pregnancy-outcomes.index', ['state' => $slug, 'search' => request('search')]) }}" class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium {{ $stateFilter === $stateKey ? 'bg-[#55B85A] text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }}">
            ...
            <div class="p-6">
                ...
                @else
                    ...
                    <div class="lg:hidden space-y-4">
                        @foreach($paginator->items() as $row)
                            @php($patient = $row['patient'])
                            <div class="rounded-xl border border-gray-100 bg-white shadow-sm p-4 {{ in_array($row['state'], [\App\Services\PregnancyOutcomeMonitoringService::STATE_LEGACY_DELIVERED, \App\Services\PregnancyOutcomeMonitoringService::STATE_LEGACY_REFERRED], true) ? 'opacity-70' : '' }}">
