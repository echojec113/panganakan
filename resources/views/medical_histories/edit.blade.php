            ...
            <form action="{{ route('medical-histories.update', $history->id) }}" method="POST" class="px-6 py-6 space-y-6">
                ...
                <div class="border-t border-gray-100 pt-6">
                    <label class="flex items-center gap-3 p-4 border border-gray-200 rounded-2xl cursor-pointer hover:border-blue-300 transition max-w-md">
                        <input type="checkbox" name="other" value="1" class="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" {{ $history->other_specify ? 'checked' : '' }} onchange="document.getElementById('otherSpecifyGroup').classList.toggle('hidden', !this.checked)" />
                        <span class="text-sm text-gray-700">Other</span>
                    </label>
                    <div id="otherSpecifyGroup" class="{{ $history->other_specify ? '' : 'hidden' }} mt-3 max-w-md">
